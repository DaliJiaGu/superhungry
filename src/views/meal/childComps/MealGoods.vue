<template>
  <div>
    <div class="goods">
      <scroll class="menu-scroll" ref="menuScroll" :probeType="probeType" @scroll="menuScroll">
      ...
      <scroll class="foods-scroll" ref="foodsScroll" :probeType="probeType" @scroll="foodsScroll">
        <div class="foods-wrapper" ref="foodsWrapper">

          <ul class="food-list" ref="foodsUI">
            <li class="food-list-hook" v-for="(item, index) in meal" :key="index">
              <h1 class="title">{{item.dishes_cname}}</h1>
              <ul>
                <li class="food-item bottom-border-1px" v-for="(item1, index) in item.foods" :key="index"
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      // 这个方式是在这个组件内直接使用第三方组件的写法，在实际开发中最好不要很多组件都直接使用第三方组件，这样依赖性太强，如果以后第三方组件不再维护或者出现问题的话，代码就需要大量的进行修改
      // this.$store.dispatch('getGoods', () => {  //这个函数是数据更新之后执行
    ...
    methods: {
      ...
      initTops() {
        ...
        // lis.push(this.$refs.foodsUI.getElementsByClassName('food-list-hook'))
        console.log(lis);
        console.log(typeof lis);
        console.log(Object.keys(lis).length);
        // for (let i = 0; i < Object.keys(lis).length; i++) {
        //   console.log(i);
